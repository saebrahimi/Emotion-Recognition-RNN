...
# totsov = trained on train, stopped on val
feature_dict_train_totsov = OrderedDict()
feature_dict_val_totsov = OrderedDict()
feature_dict_test_totsov = OrderedDict()
...
for train_id, train_feature, train_probs, train_target in zip(
    ...
    feature_dict_train_totsov[train_id] = OrderedDict()
    feature_dict_train_totsov[train_id]['convnet_features'] = train_feature
    feature_dict_train_totsov[train_id]['convnet_probs'] = train_probs
    feature_dict_train_totsov[train_id]['target'] = train_target
...
for val_id, val_feature, val_probs, val_target in zip(
    ...
    feature_dict_val_totsov[val_id] = OrderedDict()
    feature_dict_val_totsov[val_id]['convnet_features'] = val_feature
    feature_dict_val_totsov[val_id]['convnet_probs'] = val_probs
    feature_dict_val_totsov[val_id]['target'] = val_target
...
for test_id, test_feature, test_probs in zip(
    ...
    feature_dict_test_totsov[test_id] = OrderedDict()
    feature_dict_test_totsov[test_id]['convnet_features'] = test_feature
    feature_dict_test_totsov[test_id]['convnet_probs'] = test_probs
...
for train_id, train_feature, train_probs, train_target in zip(
    ...
    if not train_id in feature_dict_train_totsov.keys():
        print '{0} not in feature_dict_train_totsov'.format(train_id)
        continue
    assert train_target==feature_dict_train_totsov[train_id]['target'],\
    'audio: {0},{1},{2}'.format(
        train_target, feature_dict_train_totsov[train_id]['target'], train_id)
    feature_dict_train_totsov[train_id]['audio_features'] = train_feature
    feature_dict_train_totsov[train_id]['audio_probs'] = train_probs
...
for val_id, val_feature, val_probs, val_target in zip(
    ...
    if not val_id in feature_dict_val_totsov.keys():
        print '{0} not in feature_dict_val_totsov'.format(val_id)
        continue
    assert val_target==feature_dict_val_totsov[val_id]['target'],\
    'audio: {0},{1},{2}'.format(
        val_target, feature_dict_val_totsov[val_id]['target'], val_id)
    feature_dict_val_totsov[val_id]['audio_features'] = val_feature
    feature_dict_val_totsov[val_id]['audio_probs'] = val_probs
...
for test_id, test_feature, test_probs in zip(
    ...
    audio_probs_test):
    if not test_id in feature_dict_test_totsov.keys():
        print '{0} not in feature_dict_test_totsov'.format(test_id)
        continue
    feature_dict_test_totsov[test_id]['audio_features'] = test_feature
    feature_dict_test_totsov[test_id]['audio_probs'] = test_probs
...
for train_id, train_feature, train_probs, train_target in zip(
    ...
    'rnn: {0},{1},{2}'.format(
        train_target, feature_dict_train_totsov[train_id]['target'], train_id)
    feature_dict_train_totsov[train_id]['rnn_features'] = train_feature
    feature_dict_train_totsov[train_id]['rnn_probs'] = train_probs
...
for val_id, val_feature, val_probs, val_target in zip(
    ...
    'rnn: {0},{1},{2}'.format(
        val_target, feature_dict_val_totsov[val_id]['target'], val_id)
    feature_dict_val_totsov[val_id]['rnn_features'] = val_feature
    feature_dict_val_totsov[val_id]['rnn_probs'] = val_probs
...
for test_id, test_feature, test_probs in zip(
    ...
    if not test_id in feature_dict_test_totsov.keys():
        print '{0} not in feature_dict_test_totsov'.format(test_id)
        continue
    feature_dict_test_totsov[test_id]['rnn_features'] = test_feature
    feature_dict_test_totsov[test_id]['rnn_probs'] = test_probs
...
for train_id, train_feature, train_probs, train_target in zip(
    ...
    'activity: {0},{1},{2}'.format(
        train_target, feature_dict_train_totsov[train_id]['target'], train_id)
    feature_dict_train_totsov[train_id]['activity_features'] = train_feature
    feature_dict_train_totsov[train_id]['activity_probs'] = train_probs
...
for val_id, val_feature, val_probs, val_target in zip(
    ...
    'activity: {0},{1},{2}'.format(
        val_target, feature_dict_val_totsov[val_id]['target'], val_id)
    feature_dict_val_totsov[val_id]['activity_features'] = val_feature
    feature_dict_val_totsov[val_id]['activity_probs'] = val_probs
...
for test_id, test_feature, test_probs in zip(
    ...
    if not test_id in feature_dict_test_totsov.keys():
        print '{0} not in feature_dict_test_totsov'.format(test_id)
        continue
    feature_dict_test_totsov[test_id]['activity_features'] = test_feature
    feature_dict_test_totsov[test_id]['activity_probs'] = test_probs

modal_list.append('activity')

pickle.dump({'feature_dict_train_totsov': feature_dict_train_totsov,
             'feature_dict_val_totsov': feature_dict_val_totsov,
             'feature_dict_test_totsov': feature_dict_test_totsov,
             'modal_list': modal_list}, file('feature_dicts_totsov.pkl','w'))
