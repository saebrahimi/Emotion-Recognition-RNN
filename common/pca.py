...
# NOTE: BE CAREFUL TO REUSE num_processed_cases only in the corresponding
# function!!!! bad example: feeding num_processed_cases returned by
# compute_mean0_iteratively to compute_cov_iteratively.
...
def compute_std0_batchwise(data, use_gpu=True, batchsize=100, verbose=True):
    ...
    if use_gpu:
        s0 = theano.shared(
            np.zeros((1, ndim), dtype=theano.config.floatX), name='s0')
        s0_update_f = theano.function(
            inputs=[theano_matrix1, theano_row_vector, theano_scalar],
            outputs=[],
...
def compute_covmat_batchwise(data, use_gpu=True, batchsize=100, verbose=True):
    ...
    if use_gpu:
        ...
        update_C_f = theano.function(
            inputs=[theano_matrix1, theano_matrix2, theano_scalar],
            outputs=[],
            updates={
...
def pca(data, use_gpu=True, batchsize=100, const_norm=True, data_norm=True, verbose=True):
    ...
    if data_norm:
        m0 = compute_mean0_batchwise(
            data, batchsize=batchsize, use_gpu=use_gpu, verbose=verbose)
        for bidx in range(nbatches):
            start = bidx * batchsize
            end = min((bidx + 1) * batchsize, ncases)
            if use_gpu:
    ...
    covmat = compute_covmat_batchwise(
        data, use_gpu=use_gpu, batchsize=batchsize, verbose=verbose)
...
if __name__ == '__main__':
    import tables
    data = np.load(
        '/home/shared/data/hollywood2/l2_inputs_500000x8x300.npy', 'r')
